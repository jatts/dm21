...
/* ═══════════════════════════════════════
   PUSH NOTIFICATIONS
   OneSignal SDK-first + Capacitor fallback
═══════════════════════════════════════ */
(function() {
    ...
    function safeToast(msg, kind, ms) {
        if (typeof showToast === 'function') {
            showToast(msg, kind || 'info', ms || 4000);
            return;
        }
    ...
    async function showForegroundNotification(notification) {
        var title = notification && notification.title ? String(notification.title) : 'New notification';
        ...
        // 1) Prefer native local notification if plugin exists
        var LocalNotifications = getLocalNotificationsPlugin();
        ...
        safeToast(title + (body ? ' — ' + body : ''), 'info', 5000);
    }

    function setStoredIdentity(userId) {
        try {
            if (userId) localStorage.setItem('os_player_id', userId);
        } catch (e) {}
    }

    async function initOneSignalSdkIfAvailable() {
        var OneSignal = window.OneSignalDeferred || window.OneSignal;
        if (!OneSignal) return false;

        try {
            // OneSignal web-style deferred init (safe if the SDK is later added)
                ...
                window.OneSignalDeferred.push(async function(OS) {
                    try {
                        ...
                        try {
                            ...
                            if (name && OS.User && typeof OS.User.addTag === 'function') {
                                await OS.User.addTag('employee_name', name);
                                await OS.User.addTag('app_version', '2.1');
                            }
                        } catch (e) {}
                        ...
                        if (OS.User && typeof OS.User.getPushSubscription === 'function') {
                            try {
                                var sub = await OS.User.getPushSubscription();
                                if (sub && sub.id) setStoredIdentity(sub.id);
                            } catch (e) {}
                        }
                        ...
                        if (OS.Notifications && typeof OS.Notifications.addEventListener === 'function') {
                            OS.Notifications.addEventListener('foregroundWillDisplay', function(event) {
                                try {
                                    var n = event && event.notification ? event.notification : event;
                                    showForegroundNotification({
                                        title: n && n.title ? n.title : 'Notification',
                                        body: n && n.body ? n.body : '',
                                        data: n && n.additionalData ? n.additionalData : {}
                                    });
                                } catch (e) {
                                    console.warn('[Push] foregroundWillDisplay error:', e);
                                }
                            });
                            OS.Notifications.addEventListener('click', function(event) {
                                try {
                                    ...
                                    console.warn('[Push] notification click error:', e);
                                }
                            });
                        }

                        console.log('[Push] OneSignal SDK initialized');
                    } catch (err) {
                        console.warn('[Push] OneSignal SDK init failed:', err);
                    }
                });
                return true;
            }
        } catch (e) {
            console.warn('[Push] OneSignal SDK bootstrap error:', e);
        }

        return false;
    }

    async function initCapacitorPushFallback() {
        var PushNotifications = getPushPlugin();
        if (!PushNotifications) {
            console.warn('[Push] PushNotifications plugin nahi mila');
            return;
        }
        ...
        try {
            ...
            console.warn('[Push] Permission/register error:', e);
        }

        PushNotifications.addListener('registration', function(token) {
            console.log('[Push] FCM Token received:', token && token.value ? token.value : '(empty)');
            try { localStorage.setItem('fcm_token', token.value || ''); } catch (e) {}

            // Legacy OneSignal REST registration fallback (kept only if SDK is not available)
            // NOTE: supported OneSignal Capacitor integration should use the OneSignal SDK.
        ...
        PushNotifications.addListener('registrationError', function(error) {
            ...
            safeToast('Push registration error', 'error', 4000);
        });
        ...
        PushNotifications.addListener('pushNotificationActionPerformed', function(action) {
            ...
            try {
                ...
                console.warn('[Push] action URL open error:', e);
            }
        });
    ...
    // Legacy REST fallback kept for the current app structure.
    ...
    async function updateEmployeeTag() {
        ...
        var name = getEmployeeName();
        if (!name) {
            setTimeout(updateEmployeeTag, 3000);
            return;
        }
    ...
    async function bootPush() {
        var sdkReady = await initOneSignalSdkIfAvailable();
        if (!sdkReady) {
            await initCapacitorPushFallback();
        }
    }
    ...
    if (window.__capacitorReady) {
        ...
        window.addEventListener('capacitorPluginsReady', bootPush);
        setTimeout(bootPush, 3000);
    }
})();
